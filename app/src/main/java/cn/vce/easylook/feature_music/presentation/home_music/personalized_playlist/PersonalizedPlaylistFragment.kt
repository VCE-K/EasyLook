...
import cn.vce.easylook.databinding.FragmentPersonalizedPlaylistBinding
...
import cn.vce.easylook.feature_music.presentation.home_music.music_local.MusicLocalVM
...
@AndroidEntryPoint
class PersonalizedPlaylistFragment: BaseVmFragment<FragmentPersonalizedPlaylistBinding>() {
    private lateinit var viewModel: PersonalizedPlaylistVM
    ...
    override fun init(savedInstanceState: Bundle?) {
        initView()
        binding.run {
            lifecycleOwner = this@PersonalizedPlaylistFragment
            m = viewModel
            v = this@PersonalizedPlaylistFragment // 数据请求完成绑定点击事件
            tc = PersonalizedPlaylistEvent.TextChange
        }
        ...
        page.onRefresh {
            scope {
                withContext(Dispatchers.Main) {
                    viewModel.onEvent(PersonalizedPlaylistEvent.FetchData)
                }
            }
        }.showLoading()
    ...
    override fun getLayoutId(): Int? = R.layout.fragment_personalized_playlist
